...
<p>1층 부터 채우는 식이 아닌, 엘리베이터에서 인접한 방, 즉 101, 201, 301 그리고 모든 층을 다 배정했을 경우 102, 202, 302 ... 처럼 배치해서 계산을 해야한다. 엘리베이터를 타고 이동하는 거리는 신경쓰지 않으며, 다만 걷는 거리가 같을 때에는 아래층의 방을 선호한다는 정도 감안하고 식
을 설계하였다.</p>

<p>그래서 층(H)은 몇 번째 손님(N)에서 나머지 값이 배정된 방의 층수가 될 것이며, 호수는 몇 번째 손님에서 층수를 나눈 몫이 될 것이다. 다만 몇 번째 손님의 값이 층수보다 작을 경우에는 몇 번째 손님이 즉 층 수가 될 것이며 자연스레 호수는 1 이 되도록 설계하였다.</p>

<p>글이 많아 복잡한 문제일 줄 알았으나 자세히 읽어보니 간단한 규칙으로 풀 수 있는 가벼운 문제였던 것 같다. 다만 몇 번째 손님의 값이 층수보다 작을 경우를 생각하지 못하여 문제해결에 생각보다 시간이 소요된 문제였다.</p>
